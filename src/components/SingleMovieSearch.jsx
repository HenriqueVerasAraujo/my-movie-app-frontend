...
export default function SingleMovieSearch({ movie }) {
  ...
  return (
    <div>
        {movie.overview !== 'not render' && (
          <div onClick={ navigateMovie } className='flex h-[225px] rounded-xl w-full bg-slate-300 shadow-[5px_5px_10px_0px_rgba(0,0,0,0.3)] hover:cursor-pointer'>
            <div className='w-[20%] overflow-hidden rounded-l-xl'>
                <img src={`https://image.tmdb.org/t/p/original/${ movie.poster_path }`} alt="Poster not found" />
            </div>
            <div className='h-full w-full flex flex-col'>
              <div className='flex justify-start items-center w-full h-[20%] rounded-tr-xl bg-sky-600 p-3 pl-7'>
                  ...
                  <h1 className='mt-[5px] text-yellow-500'>{releaseDate}</h1>
              </div>
              <div className='w-full flex flex-col'>
                <div className='flex px-7 py-3 justify-start items-center'>
                  <h2 className='text-start italic'>{movie.overview}</h2>
                </div>
              </div>
            </div>
          </div>
